            ...
            while (time.time() - start) < timeout:
                ...
                try:
                    ...
                    if output:
                        ...
                        if (
                            "Connected to MQTT" in output
                            or "ReID system ready" in output
                        ):
                            ready = True
    ...
    def run_test_scenario(self, scenario_name, scenario_type):
        ...
        def on_message(client, userdata, msg):
            ...
            try:
                ...
                self.print_info(
                    f"Received on {msg.topic}: person_id={payload.get('person_id', 'N/A')}"
                )
            except Exception:
                pass
    ...
    def _verify_test_results(self, scenario_name, scenario_type):
        ...
        for topic in received_topics:
            ...
            for msg in messages:
                ...
                self.print_info(
                    f"    - person_id={person_id}, camera={camera}, confidence={confidence}"
                )

        # Verify Home Assistant sensors
        ha_last_person = self._get_ha_sensor_state(
            "sensor.frigate_identity_last_person"
        )
        ha_all_persons = self._get_ha_sensor_state(
            "sensor.frigate_identity_all_persons"
        )
...
def run_real_frigate_tests(
    ...
    if selected_event_date is not None:
        print(f"{Colors.OKBLUE}ℹ Event day (UTC): {selected_event_date}{Colors.ENDC}")
    elif selected_days_ago is not None:
        print(
    ...
    try:
        ...
        if result.returncode == 0:
            report_path = (
                Path(__file__).parent / "tests" / "output" / "real_frigate_report.html"
            )
