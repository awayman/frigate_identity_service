...
def test_frigate_api(frigate_url, api_key=None):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print_success("Frigate API is accessible")
            if cameras:
                print_info(f"Available cameras: {', '.join(cameras)}")
            return True, cameras
        elif response.status_code == 401:
            print_fail("Frigate API returned 401 (Unauthorized - check API key)")
            return False, []
        else:
...
def setup_mqtt_config():
    ...
    use_auth = (
        input("Does your MQTT broker require authentication? (y/n) [n]: ")
        .strip()
        .lower()
        == "y"
    )
...
def setup_frigate_config():
    ...
    print_info(
        "(This is optional for integration testing if you don't have API access)"
    )
    ...
    if not success:
        ...
        use_api_key = (
            input("Do you have a Frigate API key? (y/n) [n]: ").strip().lower() == "y"
        )
        if use_api_key:
...
def setup_homeassistant_config():
    ...
    if test_homeassistant(ha_url, token):
        ...
        print_warning(
            "Could not verify Home Assistant connection. Continue anyway? (y/n) [n]: "
        )
        if input().strip().lower() == "y":
            return {"url": ha_url, "token": token}
        else:
