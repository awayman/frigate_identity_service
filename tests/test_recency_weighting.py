...
sys.path.insert(
    0, os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "frigate_identity_service")
)
...
class TestRecencyWeighting:
    ...
    def test_linear_decay_fresh_embedding(self):
        ...
        assert 0.95 <= weight <= 1.0, f"Fresh embedding should have weight near 1.0, got {weight}"
    ...
    def test_linear_decay_old_embedding(self):
        ...
        assert 0.3 <= weight <= 0.35, f"Old embedding should have weight near floor (0.3), got {weight}"
    ...
    def test_linear_decay_mid_age_embedding(self):
        ...
        expected = 0.65
        assert abs(weight - expected) < 0.05, f"Mid-age embedding should have weight ~{expected}, got {weight}"
    ...
    def test_no_decay_mode(self):
        ...
        for hours_ago in [0, 12, 24, 36, 47]:
            ...
            assert weight == 1.0, f"'none' decay mode should always return 1.0, got {weight} at {hours_ago}h"
    ...
    def test_invalid_timestamp_fallback(self):
        ...
        assert weight == expected_mid, f"Invalid timestamp should return mid-range weight {expected_mid}, got {weight}"
...
class TestConfidenceWeighting:
    ...
    def test_confidence_disabled_returns_1(self):
        ...
        for confidence in [0.0, 0.5, 0.9, 1.0]:
            ...
            assert weight == 1.0, f"Disabled confidence weighting should return 1.0, got {weight}"
    ...
    def test_confidence_enabled_scaling(self):
        ...
        assert matcher._confidence_weight(0.0) == 0.7, "Confidence 0.0 should map to 0.7"
        assert matcher._confidence_weight(1.0) == 1.0, "Confidence 1.0 should map to 1.0"
        
        # Test midpoint
        mid_weight = matcher._confidence_weight(0.5)
        expected = 0.7 + (0.3 * 0.5)  # 0.85
        assert abs(mid_weight - expected) < 0.01, f"Confidence 0.5 should map to ~{expected}, got {mid_weight}"
    ...
    def test_confidence_clamping(self):
        ...
        # Test below 0
        assert matcher._confidence_weight(-0.5) == 0.7, "Negative confidence should clamp to 0.7"
        
        # Test above 1
        assert matcher._confidence_weight(1.5) == 1.0, "Confidence > 1 should clamp to 1.0"
...
class TestCombinedWeighting:
    ...
    def test_fresh_high_confidence_gets_best_weight(self):
        ...
        assert combined >= 0.95, f"Fresh high-confidence embedding should have weight near 1.0, got {combined}"
...
class TestConfigurationValidation:
    ...
    def test_invalid_decay_mode_raises(self):
        ...
        try:
            matcher = EmbeddingMatcher(decay_mode="invalid")
