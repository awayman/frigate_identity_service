...
class TestRealFrigateIntegration:
    ...
    def test_frigate_api_connectivity(
        self, frigate_session, frigate_host, frigate_config
    ):
    ...
    def test_fetch_recent_events(
        self, frigate_session, frigate_host, frigate_event_filters
    ):
    ...
    def test_fetch_snapshot_from_event(
        ...
        snapshot_bytes = fetch_snapshot_bytes(
            frigate_session, snapshot_url, crop=True, quality=85, height=400
        )

        assert snapshot_bytes is not None, (
            f"Failed to fetch snapshot from {snapshot_url}"
        )
    ...
    def test_full_e2e_pipeline_with_real_snapshots(
        ...
        logger.info(
            f"Processing {len(non_zoom_events)} events from non-zoom cameras..."
        )
        ...
        for i, event in enumerate(non_zoom_events[:5]):
            ...
            try:
                ...
                logger.info(
                    f"✓ Event {i + 1}: Extracted embedding from {camera} (event: {event_id})"
                )
        ...
        assert len(embeddings) > 0, (
            "Should have successfully processed at least one event"
        )
        logger.info(
            f"✓ Successfully extracted embeddings from {len(embeddings)} events"
        )
    ...
    def test_batch_process_recent_events(
        ...
        logger.info(
            f"Batch processing {len(non_zoom_events)} events from non-zoom cameras..."
        )
    ...
    def test_facial_recognition_and_reid_pipeline(
        ...
        # Filter out zoom camera events
        facial_events = [
            e for e in facial_events if e.get("camera", "").lower() != "zoom"
        ]
        ...
        logger.info(
            f"Loaded {len(facial_events)} facial recognition events from non-zoom cameras"
        )
        ...
        unknown_events = [
            e for e in unknown_events if e.get("camera", "").lower() != "zoom"
        ]
        ...
        for event in facial_events[:10]:  # Limit to first 10 for reasonable duration
            ...
            if not snapshot_bytes or not validate_image(snapshot_bytes):
                logger.warning(
                    f"Skipping invalid snapshot for {person_name}: {event_id}"
                )
                continue
            ...
            try:
                ...
                logger.error(
                    f"Failed to process facial recognition event {event_id}: {e}"
                )

        # Verify we learned at least one person
        assert phase1_processed > 0, (
            "Should have successfully learned at least one person"
        )
        logger.info(
            f"✓ Phase 1 complete: Learned {len(learned_persons)} persons: {sorted(learned_persons)}"
        )
        ...
        if not unknown_events:
            logger.warning(
                "No unknown person events found - skipping ReID matching test"
            )
        ...
        logger.info("=" * 80)
        logger.info("Phase 1 - Facial Recognition:")
        logger.info(f"  Processed: {phase1_processed} events")
        logger.info(f"  Learned: {len(learned_persons)} unique persons")
        logger.info("Phase 2 - ReID Matching:")
        logger.info(f"  Processed: {phase2_processed} unknown events")
        logger.info(
            f"  Matched: {reid_matches} ({reid_matches / max(phase2_processed, 1) * 100:.1f}%)"
        )
        logger.info(
            f"  No match: {no_matches} ({no_matches / max(phase2_processed, 1) * 100:.1f}%)"
        )
        logger.info("=" * 80)
        ...
        assert phase2_processed > 0 or not unknown_events, (
            "Should have processed unknown events if available"
        )
